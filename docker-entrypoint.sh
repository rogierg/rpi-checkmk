...
create_system_apache_config() {
    ...
    # echos removed in favor of cat, as during build for some reason echo would ignore the '-e' and instead write -e in the file...
    # echo -en $'# Created for Checkmk docker container\\n\\nUseCanonicalName Off\\nServerName 127.0.0.1\\n' >"$APACHE_DOCKER_CFG"
    # Redirect top level requests to the sites base url
    # echo -en $'# Redirect top level requests to the sites base url\\nRedirectMatch 302 ^/$ /$CMK_SITE_ID/\\n' >>"$APACHE_DOCKER_CFG"
    # '<<-EOF' is not working for tabs/instead of spaces so we push it to the left.
cat > $APACHE_DOCKER_CFG <<-EOF
